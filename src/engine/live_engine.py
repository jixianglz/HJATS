...
class LiveEngine:
        ...
        try:
            ...
            for order in orders:
                ...
                if oaction == "OPEN":
                    ...
                    if result["success"]:
                        # 从持仓池获取开仓价，计算盈亏
                        pos = self.order_pool.get(order["uid"], {})
            ...
            # 9. 更新状态 — 从 order_pool 计算当前持仓
            pos_side = None
            pos_size = 0.0
            pos_entry = 0.0
            for uid, pdata in self.order_pool.items():
                pos_side = pdata["side"]
                pos_size += pdata["size"]
            ...
            self.status.update_position(
                side=pos_side, size=pos_size, entry_price=pos_entry,
    ...
    def _monitor_tick(self):
        ...
        try:
            # 1. 获取最新价格
            candles = self.market.get_price_v1(
                code=self.code, count=1, frequency=self.frequency
            )
