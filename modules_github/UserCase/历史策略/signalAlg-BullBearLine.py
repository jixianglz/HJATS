...
def run(parapoll):
    ...
    if(len(BullLine_ind)>0 and len(BearLine_ind)>0):     
        if(len(BullLine_ind)>1 and BullLine>0.6 and BullLine_ind[-1]<=0.6 and FilterLine>0): 
        ...
        #if((ma10[-1]<ma60[-1]) and (ma10[-2]>=ma60[-2]) ): 
        if(len(BearLine_ind)>1 and BearLine>0.6 and BearLine_ind[-1]<=0.6 and FilterLine<0): 
    ...
    AlgParas['testcount']=AlgParas['testcount']+1   
    print("Internal Alg:"+ str(AlgParas['maxlongnum']))
    
    if(AlgParas['testcount']==-1):
        signal=1
        print("Test signal = 1")
    if(AlgParas['testcount']==-1):
        signal=-1
